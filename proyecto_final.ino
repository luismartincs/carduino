...
void loop() {
  ...
  if(accion != 0){
    int desicion = planear(accion);
    
    if(desicion == 1){
      debug("Ir a la meta");
      enviarMensaje("V: Meta");
      delay(2000);
    }else if(desicion == 2){
      debug("Se solicito ayuda");
      enviarATodos();
      enviarMensaje("V: I need a medic");
      delay(20000);
    }else{
      debug("No pudo atraparla, intentar en el siguiente ciclo");
      enviarMensaje("V: Fuck...");
    }
  }
